...
支持的 DSH 版本为 `0.1.5-rc.1`、`0.1.6-alpha.1` 及其之后的 `0.1.x` 版本（`^0.1.5-rc.1 || ^0.1.6-alpha.1`）。
...
- **实时 Univer 窗口**：修改默认自动显示在可拖动、缩放、折叠和全屏的窗口中；可在插件设置中关闭自动打开（侧边栏“插件”页面的 Univer Office 页面；0.1.6-alpha.2 之前的 DSH 版本在“设置 → 插件 → 插件配置”），不影响会话审阅卡片。
- **宿主窗口接管**：嵌入式宿主可以把实时预览和审阅卡片的全屏动作交给自己的文档窗口；独立运行的 DSH 仍保留内置窗口行为。
